# R Basics 
# R Studio is a GUI (Graphical User Interface) built on top of R
# packages are split up to system and user packages 
  #user packages must be activated

myfirstobject = 5:10 
#or <- instead of =
myfirstobject
plot(myfirstobject)
# x values are index and y values are vector name 
# ctrl + L gives empty console

library(plotrix)

?plot
?lm #accesses help function on an individual function
runif #random number generator of uniform dist
runif(8,3,10) #(n,min,max)
rnorm #random number generator of a normal dist
rnorm() 

 ### This is the course script, copy and paste this into 
#RStudio. We will be going through this material during the course

### Understanding Functions in R

# setting a seed to make a random sample reproducible,
# choose any number you want
set.seed(65)

# with argument name, exact argument order
runif(n = 9, min = 3, max = 6)

set.seed(65) #creates the algos 65th random order
...
runif(2, 5, 23) # this means n=3, max=9
...
runif(3,,77)
runif(3,,15)
#cant run with out argument 1 (n) or argument 3(max)
#produces NAs

#r-tutorials.com
  #exercise database (contains questions to work through)

#download (install.packages()) #once
#activate (library()) #every session
...
(5-3)^3
 ...
 # Creating objects
x <- c(4,5,6) #lowercase c represents concatenation
#grouping all values together
...
paste("xyz", c(2,5,7,"test", 4.5)) # random vector 
...
# 1 . Create the object "myobject" 
#and assign the values 1:10
...
sum (myobject) 
# 3. Create the following vector by using 
#the paste function
...
# 4. vector of 1,2,3 : repeat the vector to get
#11 x 1, 10 x 2, and 10 x 3
x <- c(1,2,3)
x
...
#import data file in environment
...
plot(lynx, main="Lynx Trappings", col="blue",
     col.main=62, cex.main=1.5)

# label names
plot(lynx, ylab="Lynx Trappings", xlab="LYNX")
